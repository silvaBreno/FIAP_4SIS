...
cap = cv2.VideoCapture('livro_video.mp4')
...
while True:
    ...
    gray1 = cv2.drawKeypoints(frame, kp1, outImage=np.array([]), flags=0)
    gray2 = cv2.drawKeypoints(img1, kp2, outImage=np.array([]), flags=0)
    ...
    img3 = cv2.drawMatches(frame,kp1,img1,kp2,matches[:8],None, flags=2)
